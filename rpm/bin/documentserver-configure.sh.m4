...
restart_services() {
	...
	[ -a /etc/nginx/conf.d/univaultoffice-documentserver.conf ] && \
...
establish_redis_conn() {
	...
	exec {FD}<> /dev/tcp/$REDIS_HOST/${REDIS_PORT:="2025"} && exec {FD}>&-
...
establish_amqp_conn() {
	...
	exec {FD}<> /dev/tcp/$AMQP_SERVER_HOST/${AMQP_SERVER_PORT:="2025"} && exec {FD}>&-
